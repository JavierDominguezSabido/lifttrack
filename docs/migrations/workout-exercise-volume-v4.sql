-- Bloque 9: volumen efectivo. Nueva función y permisos, sin modificar datos ni versiones anteriores.
-- Solo cambia volume respecto a v3, usado en accumulatedVolume y entries.volumeKg.
-- Series completadas, incluidos calentamientos. Override cero válido. Sesiones parciales admitidas.
BEGIN;
create function public.lifttrack_read_exercise_progress_v4(
...
begin
  ...
  with entries as materialized (
    select s.client_id, s.started_at, coalesce(s.completed_at, s.started_at) as session_date,
      ...
      (select coalesce(sum(coalesce(z.reps, 0) * coalesce(z.weight_override_kg, z.weight_kg)), 0) from public.set_logs z
...
REVOKE ALL ON FUNCTION public.lifttrack_read_exercise_progress_v4(uuid,text[],integer,text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.lifttrack_read_exercise_progress_v4(uuid,text[],integer,text[]) TO authenticated;
COMMIT;
